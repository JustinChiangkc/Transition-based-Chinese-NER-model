...
def transition(model, input_is, args=None, debug=False):
    #input_is => str
    model = model.eval()
    ...
    while terminate == False:
        if debug == True:
            ...
            #print('dep_parsed_graph', dep_parsed_graph)
        ...
        if len(stack_state) == 0:
            ...
            if debug == True:
                print('pred_action after len(stack)<=1, R_mask', pred_action)
        if len(buffer_state) == 0: #and len(stack) != 1:
            #cannot do SH,O
            pred_action = pred_action * SH_mask
            pred_action = pred_action * O_mask
            if debug == True:
                print('pred_action after len(buffer_state)==0, SH_mask', pred_action)
        if debug == True:
        ...
        if action.startswith('O'):
            ...
            ner_result.append([[entity_s,entity_e-1],a_entity,action.split('-')[-1].split('.')[0]])
            stack_state = []
            entity_s = entity_e 
